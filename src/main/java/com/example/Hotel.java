...
public class Hotel {
  ...
  private void removeReservation(String name, String type) {
    ...
    for (int i = 0; i < numOfReservations; i++) {
      ...
      if (reservations[i].getName().equalsIgnoreCase(name) &&
        ...
        // one place to the left to remove the found reservation from the reservation
        // array
    ...
    throw new NoSuchElementException(
        "There is no reservation for a " +
            type +
            " room under the name of " +
            name +
            ".");
  }
  ...
  public void createReservation(String name, String type) {
    ...
    if (room == null) {
      System.out.println(
          "Sorry " + name + ", we have no available rooms of the desired type.");
    } else {
      ...
      if (numOfReservations < reservations.length) {
        ...
        System.out.println(
            "You have successfully reserved a " +
                type +
                " room under the name " +
                name +
                ". We look forward having you at " +
                this.name);
      } else {
    ...
    try {
      ...
      System.out.println(
          name +
              ...
              type +
              "room has been successfully cancelled.");
    } catch (NoSuchElementException ex) { // catch the thrown exception if no reservation found
  ...
  public void printInvoice(String name) {
    ...
    // print the invoice
    System.out.println(
        name + "'s invoice is of $" + String.format("%.1f", total));
  }
